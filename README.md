...
* [emoji.json](dictionaries/emoji.json) (automatically generated)
...
* [misstrokes.json](dictionaries/misstrokes.json)† (automatically generated)
* [modifiers-single-stroke.json](dictionaries/modifiers-single-stroke.json)
...
> [!IMPORTANT]
> Rather than using my JSON emoji dictionary, I recommend installing the [Plover Emoji](https://github.com/panathea/plover_emoji) plugin using the [Plover plugin manager](https://plover.wiki/index.php/Plugins) to write *any* emoji and create personalised single-stroke briefs for the emoji you use most often.

The [`emoji.json`](dictionaries/emoji.json) is auto-generated by the [Typey Type CLI](https://github.com/didoesdigital/typey-type-cli). The approach is based on [Thea Morin's emoji dictionary](https://www.openstenoproject.org/stenodict/dictionaries/emoji.html).

> [!WARNING]
> The auto-generated dictionary is based on emoji that existed in 2015. If it were updated to use all the latest emoji in existence, it would be almost 200k entries and 22MB larger. That would take a long time to load in steno engines like Plover. It would be too big to load into Typey Type (using local storage) and Typey Type would become too slow. At this point, I only fix misstrokes in this dictionary but otherwise treat it as unmaintained.
...
You can check the [Typey Type CLI source code `customBriefs`](https://github.com/didoesdigital/typey-type-cli/blob/main/src/lib/stemoji.ts#L212-L252) for the current additions.
...
- [`markdown.json`](https://github.com/didoesdigital/steno-dictionaries/blob/master/dictionaries/markdown.json) is based on Thea's [Markdown dictionary](http://www.openstenoproject.org/stenodict/dictionaries/markdown.html) with additional entries like fenced code blocks with languages specified, including `js`, `css`, `html`, `bash`, and `vim`.
